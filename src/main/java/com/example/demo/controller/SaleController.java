...
public class SaleController {
    ...
    public String addSale(@ModelAttribute Sale sale, RedirectAttributes ra) {
        try {
            ...
            if (product == null || sale.getQuantity() == null || sale.getQuantity() <= 0) {
                ra.addFlashAttribute("error", "Invalid product or quantity");
                return "redirect:/sales";
            }
            if (product.getQuantity() == null || sale.getQuantity() > product.getQuantity()) {
                ra.addFlashAttribute("error", "Insufficient stock (Available: "
            ...
            ra.addFlashAttribute("success", "Sale recorded and stock updated!");
        } catch (Exception e) {
    ...
    public String showEditSaleForm(@PathVariable Integer id, Model model) {
        ...
        model.addAttribute("products", productService.getAllProducts());
        model.addAttribute("customers", customerService.getAllCustomers());
        return "edit-sale"; // Ensure this template exists or create it
    }
    ...
    public String updateSale(@PathVariable Integer id, @ModelAttribute Sale sale, RedirectAttributes ra) {
        try {
            ...
            Product product = productService.getProductById(sale.getProduct().getId());
            if (sale.getUnitSellingPrice() == null)
                sale.setUnitSellingPrice(product.getPrice());
            sale.setTotalPrice(sale.getUnitSellingPrice() * sale.getQuantity());

            saleService.updateSale(sale);
            ra.addFlashAttribute("success", "Sale updated and stock adjusted!");
        } catch (Exception e) {
            ra.addFlashAttribute("error", "Error: " + e.getMessage());
        }
        return "redirect:/sales";
    ...
    public void generateReceipt(@PathVariable Integer id, HttpServletResponse response)
        ...
        buyerTable.addCell(new Phrase(
                "Buyer Name: " + (sale.getCustomer() != null ? sale.getCustomer().getName() : "Walk-in"), normalFont));
        buyerTable.addCell(new Phrase("Invoice No: " + sale.getId(), normalFont));
        buyerTable.addCell(
                new Phrase("Buyer PAN: " + (sale.getCustomer() != null && sale.getCustomer().getPanNumber() != null
                        ? sale.getCustomer().getPanNumber()
                        : "-"), normalFont));
        buyerTable.addCell(new Phrase("Date: " + sale.getDate(), normalFont));
        buyerTable.addCell(
                new Phrase("VAT No: " + (sale.getCustomer() != null && sale.getCustomer().getVatNumber() != null
                        ? sale.getCustomer().getVatNumber()
                        : "-"), normalFont));
        buyerTable.addCell(new Phrase(
                "Contact: " + (sale.getCustomer() != null ? sale.getCustomer().getContact() : "-"), normalFont));
        doc.add(buyerTable);
        doc.add(new Paragraph(" "));
        ...
        double unitPrice = (sale.getUnitSellingPrice() != null) ? sale.getUnitSellingPrice()
                : (sale.getProduct() != null && sale.getProduct().getPrice() != null ? sale.getProduct().getPrice()
                        : 0.0);
        ...
        doc.add(table);
        doc.add(new Paragraph(" "));
        doc.add(new Paragraph("Amount in words: " + NumberToWordsConverter.convert((long) gross) + " only.",
                normalFont));
        doc.add(new Paragraph(" "));
