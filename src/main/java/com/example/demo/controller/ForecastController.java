...
public class ForecastController {
    ...
    @GetMapping
    public String showForecasts(Model model) {
        try {
    ...
    public String generateForecast(@PathVariable Integer productId, RedirectAttributes ra) {
        try {
    ...
    public String generateAllForecasts(RedirectAttributes ra) {
        try {
            List<Product> products = productService.getAllProducts();
            if (products.isEmpty()) {
                ra.addFlashAttribute("error", "No products found!");
                return "redirect:/forecasts";
            }
            ...
            if (successCount > 0)
                ...
                ra.addFlashAttribute("error", "No forecasts generated.");
        } catch (Exception e) {
            ra.addFlashAttribute("error", "Error: " + e.getMessage());
        }
        return "redirect:/forecasts";
